...
pub async fn get_process(
    ...
    let row = sqlx::query_as::<_, BusinessProcessDetailRow>(
        ...
        FROM business_processes bp
        LEFT JOIN users uo ON uo.user_id = bp.owner_user_id
        LEFT JOIN users ust ON ust.user_id = bp.steward_user_id
        LEFT JOIN process_categories pc ON pc.category_id = bp.category_id
        LEFT JOIN business_processes pp ON pp.process_id = bp.parent_process_id
...
pub async fn create_process(
    ...
    // NOTE: process_code is auto-generated by DB trigger when NULL
...
pub async fn update_process(
    ...
    let process = sqlx::query_as::<_, BusinessProcess>(
        ...
        SET process_name          = COALESCE($1, process_name),
            ...
            steward_user_id       = COALESCE($7, steward_user_id),
            is_critical           = COALESCE($8, is_critical),
            criticality_rationale = COALESCE($9, criticality_rationale),
            frequency             = COALESCE($10, frequency),
            regulatory_requirement = COALESCE($11, regulatory_requirement),
            sla_description       = COALESCE($12, sla_description),
            documentation_url     = COALESCE($13, documentation_url),
            updated_by            = $14,
            updated_at            = CURRENT_TIMESTAMP
        WHERE process_id = $15 AND deleted_at IS NULL
        RETURNING
            process_id, process_name, process_code, description,
            detailed_description, category_id, status_id, owner_user_id,
            steward_user_id,
