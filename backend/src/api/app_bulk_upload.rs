...
const INSTRUCTIONS: &[(&str, &str, &str, &str, &str)] = &[
    ("Application Name",      "The official name of the business application",         "Yes",         "256",  "Must be unique"),
    ("Description",            "Clear description of what the application does",        "Yes",         "4000", "Plain text only"),
    ("Classification",         "Application classification category",                   "Yes",         "",     "Select from dropdown"),
    ("Vendor",                 "Name of the software vendor",                           "Yes",         "256",  ""),
    ("Vendor Product Name",    "Commercial product name from the vendor",               "Yes",         "256",  ""),
    ("Deployment Type",        "How the application is deployed",                       "Yes",         "",     "ON_PREMISE, CLOUD, HYBRID, or SAAS"),
    ("Lifecycle Stage",        "Current stage of the application lifecycle",             "Yes",         "",     "Select from dropdown"),
    ("License Type",           "Type of software license",                              "Yes",         "256",  "e.g. Perpetual, Subscription, Open Source"),
    ("Business Owner",         "Email address of the business owner",                   "Yes",         "",     "Must exist in the system"),
    ("Technical Owner",        "Email address of the technical owner",                  "Yes",         "",     "Must exist in the system"),
    ...
    ("CBA Flag",               "Whether this is a Critical Business Application",       "No",          "",     "TRUE or FALSE"),
    ("CBA Rationale",          "Justification for CBA designation",                     "Conditional", "4000", "Required if CBA Flag = TRUE"),
    ("External Reference ID",  "External system reference identifier",                  "Yes",         "256",  "e.g. CMDB ID, ServiceNow CI"),
];
...
pub async fn download_app_template(
...
async fn generate_template(pool: &PgPool) -> AppResult<Vec<u8>> {
    ...
    let (
        ...
        fetch_names(pool, "SELECT classification_name FROM application_classifications ORDER BY display_order"),
        fetch_names(pool, "SELECT stage_name FROM application_lifecycle_stages ORDER BY display_order"),
        fetch_names(pool, "SELECT unit_name FROM organisational_units ORDER BY display_order"),
        fetch_names(pool, "SELECT email FROM users WHERE is_active = TRUE ORDER BY display_name"),
    )?;
    ...
    let deployment_validation = DataValidation::new()
        .allow_list_strings(&["ON_PREMISE", "CLOUD", "HYBRID", "SAAS"]).map_err(xlsx_err)?
        .set_error_title("Invalid value").map_err(xlsx_err)?
        .set_error_message("Please enter ON_PREMISE, CLOUD, HYBRID, or SAAS.").map_err(xlsx_err)?;
    ...
    // ===== Sheet 1: Applications =====
    {
        ...
        for (col, &hdr) in TEMPLATE_HEADERS.iter().enumerate() {
            ...
            app_sheet.write_string_with_format(0, col, hdr, fmt).map_err(xlsx_err)?;
...
pub async fn bulk_upload_apps(
    ...
    for (row_num, cols) in &data_rows {
        let row_result = process_app_row(
            &ctx,
            *row_num,
            cols,
            &mut created_app_ids,
...
async fn process_app_row(
    ...
    let lifecycle_stage_id = resolve_optional_lookup(
        ctx.pool, row_num, "Lifecycle Stage", &lifecycle_opt,
    ...
    let business_owner_id = resolve_user_by_email(ctx.pool, row_num, "Business Owner", &business_owner_email, &mut row_errors).await;
    let technical_owner_id = resolve_user_by_email(ctx.pool, row_num, "Technical Owner", &technical_owner_email, &mut row_errors).await;
